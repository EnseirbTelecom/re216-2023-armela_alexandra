...
// MULTICAST_CREATE
int multi_create(char buff[MSG_LEN],int sockfd_server,char my_nickname[NICK_LEN], char my_salon[NICK_LEN]){
	char *infos = strtok(buff, " ");

	if (infos== NULL){
		printf("No space between <create> and <chanel_name> \n");
		return 0;
	}
	...
	if (infos== NULL){
		printf("No chanel_name\n");
		return 0;
	}

	// Infos stocke le nom du salon 
	char * new_chanel_name = infos;
	int len_new_chanel_name =  strlen(new_chanel_name);

	if ((len_new_chanel_name<3)||(len_new_chanel_name>127)){
		printf("The new chanel name size is incorrect; it should be between 3 and 127 characters.");
		return 0;
	}

	if (containsOnlyAlphanumeric(new_chanel_name)==0){
		printf("The new chanel name should only contain letters of the alphabet or numbers.");
		return 0;
	}
	return send_struct(sockfd_server,my_nickname,MULTICAST_CREATE,new_chanel_name,my_salon);
	
}

//MULTICAST_LIST 
int multicast_list(char buff[MSG_LEN], int sockfd_server, char my_nickname[NICK_LEN], char my_salon[NICK_LEN]){
	// printf("who : %s\n",my_nickname);
	if(send_struct(sockfd_server,my_nickname,MULTICAST_LIST,"\0","\0")==0)
		return 0;
...
//MULTICAST_JOIN 
int multicast_join(char buff[MSG_LEN], int sockfd_server, char my_nickname[NICK_LEN], char my_salon[NICK_LEN]){
	char *infos = strtok(buff, " ");

	if (infos== NULL){
		printf("No space between /join and <channel_name> \n");
		return 0;
	}
...
int multicast_join(char buff[MSG_LEN], int sockfd_server, char my_nickname[NICK_LEN], char my_salon[NICK_LEN]){
	...
	if (infos== NULL){
		printf("No space between /quit and <channel_name> \n");
		return 0;
	}
	...
	return send_struct(sockfd_server,my_nickname,MULTICAST_CREATE,chanel_name_quited,my_salon);
	
}

// MULTICAST_SEND 
int multicast_send(char buff[MSG_LEN], int sockfd_server, char my_nickname[NICK_LEN], char my_salon[NICK_LEN]){
	// Infos stocke le nom du salon sur lequel on veut envoyer un message 
	char *infos = strtok(buff, " ");

	if (infos == '\0'){
		printf("No <channel_name> indicated. \n");
		return 0;
	}
	//Récupère le nom du salon 
	infos = strtok(NULL, " ");
	char *chanel_name = infos;
	
	return send_struct(sockfd_server,my_nickname,MULTICAST_CREATE,infos,my_salon);
	
}


int echo_client(int sockfd_active,int sockfd_server, int sockfd_entree,char my_nickname[NICK_LEN], char my_salon[NICK_LEN]) {
	struct message msgstruct;
	char buff[MSG_LEN];
	int n;
	...
	if (sockfd_active==sockfd_entree)
	{
		...
		if (strncmp(buff, "/create ", strlen("/create "))==0)
			return multi_create(buff,sockfd_server,my_nickname,my_salon);

		//MULTICAST_LIST
		if (strncmp(buff, "/channel_list ", strlen("/channel_list "))==0)
			return multicast_list(buff,sockfd_server,my_nickname,my_salon);
		
		//MULTICAST_JOIN
		if (strncmp(buff, "/join  ", strlen("/join  "))==0)
			return multicast_join(buff,sockfd_server,my_nickname,my_salon);
		
		//MULTICAST_QUIT
		if (strncmp(buff, "/quit ", strlen("/quit "))==0)
			return multicast_quit(buff,sockfd_server,my_nickname,my_salon);

		//MULTICAST_SEND
		if (strncmp(buff, "/quit ", strlen("/quit "))==0)
			return multicast_send(buff,sockfd_server,my_nickname,my_salon);
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		if ((fds[0].revents & POLLIN) == POLLIN) {
			ret=echo_client(fds[0].fd,fds[0].fd,fds[1].fd,my_nickname, my_salon);
			if (ret==0){
				perror("echo_client");
				exit(EXIT_FAILURE);
			}
		...
		if ((fds[1].revents & POLLIN) == POLLIN) {
			ret=echo_client(fds[1].fd,fds[0].fd,fds[1].fd,my_nickname, my_salon);
			if (ret==0){
				perror("echo_client");
				exit(EXIT_FAILURE);
			}
